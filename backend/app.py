...
def delete_appointment(appointment_id):
    with psycopg.connect(**get_db_config()) as conn:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM citas WHERE id = %s", (appointment_id,))
...
@app.post("/admin/appointment/<int:appointment_id>/delete")
def delete_appointment_route(appointment_id):
    deleted = delete_appointment(appointment_id)
    if deleted:
        return redirect(url_for("admin_panel", message="Cita eliminada correctamente."))
    return redirect(url_for("admin_panel", message="No se encontró la cita a eliminar."))
